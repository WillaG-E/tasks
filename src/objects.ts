...
export function isCorrect(question: Question, answer: string): boolean {
    const editedQuestion = question.expected.trim().toLowerCase();
    const editedAnswer = answer.trim().toLowerCase();
    return editedQuestion === editedAnswer;
}
...
export function toShortForm(question: Question): string {
    const stringQuestion = question.id + ": " + question.name.slice(0, 10);
...
export function toMarkdown(question: Question): string {
    let beginningLines = "# " + question.name + "\n" + question.body;
    if (question.type !== "multiple_choice_question") {
        return beginningLines;
    }
...
export function publishQuestion(question: Question): Question {
    const invertedPublish = { ...question, published: !question.published };
